...
class XUIApi:
    ...
    def _base_candidates(self) -> List[str]:
        ...
        if "/panel" in BASE_URL:
            bases.append(BASE_URL.split("/panel", 1)[0])
        seen = set()
        out: List[str] = []
        for b in bases:
            b = b.rstrip("/")
    ...
    def list_inbounds(self) -> List[Dict[str, Any]]:
        endpoints = [
            ("GET", ["/panel/api/inbounds/list", "/api/inbounds/list", "/xui/API/inbounds/"]),
            ("POST", ["/panel/api/inbounds/list", "/api/inbounds/list"]),
            ("GET", ["/panel/api/inbounds/get/all", "/api/inbounds/get/all"]),
            ("POST", ["/panel/api/inbounds/get/all", "/api/inbounds/get/all"]),
        ]
