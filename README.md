...
- Webhook registration can be run explicitly with `./scripts/set_webhook.sh .`; installer primary menu also has "Set Telegram webhook now".
- If `WEBHOOK_BASE_URL` is left empty and `DOMAIN` is provided during install, installer auto-sets `WEBHOOK_BASE_URL=https://<DOMAIN>`.
