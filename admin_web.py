...
def admin_topups():
    if not auth_ok(request):
        return "Forbidden", 403
    return render_template_string(TOPUPS, style=BASE_STYLE, token=request.args.get("token"), rows=db.list_topup_requests(limit=200))
...
def admin_plans():
    ...
    return render_template_string(PLANS, style=BASE_STYLE, token=request.args.get("token"), rows=db.list_plan_templates("agent"))
